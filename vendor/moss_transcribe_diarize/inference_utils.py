...
class GenerationCancelled(RuntimeError):
    """Raised after cooperative generation cancellation."""
...
class CancellationStoppingCriteria(StoppingCriteria):
    ...
    def __call__(self, input_ids, scores, **kwargs):
        cancelled = bool(self.callback())
        return torch.full((input_ids.shape[0],), cancelled, device=input_ids.device, dtype=torch.bool)
    ...
    for message in messages:
        ...
        for item in content:
            if item.get("type") != "audio":
                continue
            audio = next(
                (item[key] for key in ("audio", "audio_url", "url", "path") if item.get(key) is not None),
                None,
            )
            if audio is None:
...
def build_transcription_messages(
    ...
    audio = str(audio_path) if isinstance(audio_path, (str, Path)) else audio_path
    return [
        {
            "role": "user",
            "content": [
